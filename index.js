...
function nextNewYear() {
  ...
  let diff = Date.parse(newYear) - Date.parse(today);
  ...
  const miliseconds = diff - (seconds * 1000);
  // console.log(miliseconds);
  return `До нового року залишилося ${days}днів, ${hours}годин, ${minutes}хвилин, ${seconds}секунд та ${miliseconds}мілісекунд`
}
...
function addDays(date, days) {
  ...
  if (date.getTime()) {
    ...
    if (typeof days === 'number') {
      ...
      let newDate = currentDate + days * 60 * 60 * 24 * 1000;
...
function getAge(birthDate) {
  ...
  // Вираховування різницю років між поточною датою та датою народження.
    let result = currentDate.getFullYear() - birthDate.getFullYear(); 
    // console.log(result);
  // Перевірка, чи поточна дата ще не пройшла день народження, тобто поточний місяць має бути менше, а якщо дорівнює дата має бути менше.
    if (birthDate.getMonth() > currentDate.getMonth()) {
        result -= 1;
        // console.log('Місяць меньший')
    } else {
      if (birthDate.getMonth() == currentDate.getMonth()) {
        if (birthDate.getDate() > currentDate.getDate()) {
          result -= 1;
          // console.log('День меньший')
        } }
    }
    // console.log(result);
...
// console.log(currentDate.toISOString()); // виводить у форматі ISO (рррр-мм-дд час з мілісекундами і все це по Грінвичу)
// console.log(currentDate.toString()); // виводить у форматі строки (день тижня, місяць, число, рік, час, часовий пояс та опис часового поясу)
// console.log(currentDate.toUTCString()); // вивід у форматі UTC за Грінвичем (день тижня, число, місяць, рік, час з секундами)
// console.log(new Date(date));
// const newYear = new Date(3600000); // створили дату за кількістю мілісекунд в 1 годині
// console.log(newYear); // результат 1 січня 1970року 01 година 00 хвилин 00 сек
// console.log(zone); // виводить різницю з часом по Гринвичу в мінутах (Гринвіч - час користувача)

const thisDay = new Date(2023, 9, 5, 18, 30, 20, 0); // встановлює дату з часом по Гринвичу, хоча вхідний параметр приймає за поточним часовим поясом (в моєму випадку часовий пояс +3)
const areaDay = new Date(Date.UTC(2023, 9, 5, 18, 30, 20, 0)); // таким чином встановиться час котрий вказаний
// console.log(thisDay.getTime());
// console.log(areaDay.getUTCHours());
// console.log(areaDay.getHours());
// метод getTime() - повертає час в мілісекундах з 01-01-1970 === setTime(value) - встановлює час за значенням value (в мілісекундах) після 01-01-1970

// методи getHours()  or  getUTCHours() - повертають години в поточному часовому поясі або в Гринвічі
// методи setHours()  or setUTCHours() - встановлють години в поточному часовому поясі або в Гринвічі

// метод getTimezoneOffset() - повертає зсув поточного часу відносно Гринвіча в мінутах. Грінвіч - Поточний (для Києва результат буде: -180)

// Date.parse(date) - конвертує дату у число мілісекунд починачи з 01-01-1970, зручно для порівняння та інших операцій обчислення
